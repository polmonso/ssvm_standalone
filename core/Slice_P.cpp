...
void Slice_P::addLongRangeEdges()
{
  ...
  #pragma omp parallel for
#endif
  for(long sid = 0; sid < (long)nSupernodes; ++sid) {
    map<sidType, supernode* >::const_iterator it = _supernodes.find(sid);
    supernode* s = it->second;
...
void Slice_P::exportSupernodeLabelQualityFile(const char* output_filename,
                                              const char* mask_dir,
                                              const int nClasses)
{
  assert(nClasses<3); // this function is only valid for two classes
  ...
  ofstream ofs_label(output_filename);
  ulong index = 0;
  int node_label = 0;
  ulong sliceSize = getWidth()*getHeight();
  ulong width = getWidth();
  const map<int, supernode* >& _supernodes = getSupernodes();
  for(map<int, supernode* >::const_iterator its = _supernodes.begin();
      its != _supernodes.end(); its++) {

    supernode* s = its->second;
    node n;
    ...
    while(!ni.isAtEnd()) {
      ni.get(n);
      ni.next();
      index = (n.z*sliceSize) + (n.y*width) + n.x;
      if(mask_data[index] == BACKGROUND_MASKVALUE) {
        node_label = BACKGROUND;
      } else {
        node_label = FOREGROUND;
      }
      ++counts[node_label];
    }

    ofs_label << (int)s->getLabel();
    ...
    //ofs_label << label << " " << its->second->size() << endl;
  }
  ofs_label.close();
  delete[] mask_data;
}
